...
export default function WorldView() {
  ...
  const [, params] = useRoute("/world/:worldId");

  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token) {
      toast.error("Please log in to view worlds");
      setLocation("/");
    }
  }, [setLocation]);
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-4 py-20">
        <div className="max-w-2xl w-full text-center">
          <h2 className="text-5xl font-bold mb-4 text-white">
            World: <span className="neon-text-cyan capitalize">{worldId.replace(/-/g, ' ')}</span>
          </h2>
          <p className="text-gray-400 text-xl mb-8">
            World implementation coming soon
          </p>
          <div className="bg-[#0d1230] border border-gray-800 rounded-xl p-12 mb-8">
            ...
            <p className="text-gray-500">
              We're building amazing experiences for you. Check back soon!
            </p>
          </div>
          <Button
            onClick={() => setLocation("/home")}
