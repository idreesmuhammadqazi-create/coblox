/**
 * Character Routes (SQLite version)
 */
...
router.post('/character', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // Check if character name already exists (case-insensitive handled in DB query)
    const existingCharacter = UserModel.getUserByCharacterName(trimmedName);
    ...
    const updatedUser = UserModel.updateUser(userId, { characterName: trimmedName });
...
router.patch('/character/avatar', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const updatedUser = UserModel.updateUser(userId, { avatarAppearance: avatarData });

    return res.status(200).json({
      success: true,
      avatarAppearance: updatedUser?.avatarAppearance,
    });
  } catch (error) {
